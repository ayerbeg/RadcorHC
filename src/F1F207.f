...
c
c subroutine F1F2IN07. Returns inelastic F1, F2, and R for
...
c 
c subroutine F1F2QE07. Returns quasi-elastic F1, F2 for
c      nucleus with charge Z and atomic number A
c      for given value of Q2 and W**2
c 
C=======================================================================
                                                                        
      SUBROUTINE F1F2IN07(Z, A, QSQ, Wsq, F1, F2, Rc)                
!--------------------------------------------------------------------
...
! convert from per nucleon to per deuteron
        if(Z .lt. 0.5) then
          call resmodd07(wsq,qsq,xvald0,F1d)
          F1p = F1d * 2.0 - F1p
        endif
        W1 = F1p / PM 
...
c get Fermi-smeared R from Erics proton fit
        call pind07(Wsq, Qsq, F1c, Rc, sigt, sigl)
c get fit to F1 in deuteron, per nucleon
        call resd07(qsq, wsq, xvald0, F1d)
...
! assume this is 2 * pf * qv
        ...
        do ism = 1,15
          ...
          IF(WSQP.GT. 1.159) THEN
            ...
            call resmodd07(wsqp,qsq,xvald0,F1dp)
            F1d = F1d + F1dp * FY(ism)
            F1p = F1p + F1pp * FY(ism)
      ...
      subroutine pind07(W2,Q2,F1,R,sigt,sigl)
...
c Need avp2f term to get right behavior x>1! 
     ...
     >    qv**2 + 2. * qv * pz - avp2f(ism)
      ...
      subroutine resd07(q2,w2,xval,F1)
...
! Do fast 20 bins if abvoe threshold
     ...
     >    qv**2 + 2. * qv * avpz(ism) - avp2(ism)
        if(w2p.gt.1.155) then
          call resmodd07(w2p,q2,xval,sigp)
...
! took out again, then put back in again
      call mec07(1.D0,2.D0,q2,w2,f1m,f2m,xval)
      if(usemec) f1 = f1 + f1m
      return
      end

      subroutine resder07(q2,w2,xval,F1er)
...
! for given W2, Q2
      SUBROUTINE RESMODD07(w2,q2,xval,sig) 
...
c Fit to proton F1, R, sigma_T, and Sigma_L from
      ...
      SUBROUTINE christy507_repeat(W2,Q2,F1,R,sigt,sigl)
c   M.E. Christy and P.E. Bosted, ``Empirical Fit to Precision 
c    Inclusive Electron-Proton Cross Sections in the Resonance Region'',
c    (arXiv:0712.3731). To be submitted to Phys. Rev. C.
     ...
     & 0.18951E+01,0.51376E+00,0.00000E+00,0.42802E+01,0.00000E+00 /
      ...
      SUBROUTINE RESMOD507_repeat(sf,w2,q2,xval,sig) 
...
CCC    Finish resonances / start non-res background calculation   CCC
 ...
 1000  format(8f12.5)
      ...
      subroutine mec07(z,a,q2,w2,f1,f2,xval)
...
!---------------------------------------------------------------------
      ...
      REAL FUNCTION FITEMC07(X,A,GOODFIT)       
!---------------------------------------------------------------------  
