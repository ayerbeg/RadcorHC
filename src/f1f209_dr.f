...
c    (arXiv:0711.0159), published in Phys. Rev. C 77, 065206 (2008). (
...
c        q2 = 1.0 * iq
c        w2 = am**2
c         F1F2QE09(1.D0, 2.D0, q2, w2, F1dqe, F2dqe)
...
C=======================================================================
      ...
      SUBROUTINE F1F2IN09dr(Z, A, QSQ, Wsq, F1, F2, rc)                       
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
      ...
      REAL*8 P(0:23)
      COMMON/PARCORR/P
      real*8 Z,A,qsq,wsq,w,f1c,x,ft,fr
      real*8 avgn,r,dr,nu,eps,kappa,sigres,flux,siglp,sigtp,F1pp,F1dp
      REAL*8 W1,W2,sigt,rc,w1pb,w2pb,F1,F2,sigl,F1d, F1p,F1n,qv
      REAL*8 W1p,W2P,DW2DPF,WSQP,pf,kf,es,dw2des,fyuse
      real a4, x4, fitemcdr, emcfac
      logical goodfit
      INTEGER ISM
...
! fr is a correction factor for Rd, based on E06-009 data  !

c      fr =   0.028*wsq/(1.+0.0025*wsq*sqrt(wsq))  !!! matches R1990  !!!

      call rdcorr(wsq,qsq,fr)

c      fr = 0.

c      fr =   fr + 0.028*wsq/(1.+0.0025*wsq*sqrt(wsq))  !!! matches R1990  !!!

c      fr = 0.050*wsq/(1.+0.0025*wsq*wsq*sqrt(wsq))
c      fr = fr*qsq/(1.+0.02*qsq*qsq*qsq*sqrt(qsq)) 

c      fr = fr*qsq/(1.+0.055*qsq*qsq*sqrt(qsq)) 

      IF(IA .lt. 2 .and. wsq.gt.1.155) THEN
        call CHRISTY507dr(Wsq,Qsq,F1p,Rc,sigt,sigl)
! If neutron, subtract proton from deuteron. Factor of two to
! convert from per nucleon to per deuteron
        if(Z .lt. 0.5) then
          call resmodddr(wsq,qsq,xvald0,F1d)

          F1n = F1d * 2.0 - F1p  !! neutron F1

          Rc = Rc*(1.+fr)*(1.+F1n/F1p)-Rc 
          Rc = Rc*F1p/F1n        !! neutron R

!! Above is neutron R with E06-009 correction !!
...
c get Fermi-smeared R from Erics proton fit
        call pinddr(Wsq, Qsq, F1c, Rc, sigt, sigl)

        Rc = Rc*(1.+fr)     !!! E06-001 correction for R_d.  Also need to adjust R_n!!!

c get fit to F1 in deuteron, per nucleon
        call resddr(qsq, wsq, xvald0, F1d)
         
c        F1d = (1.-ft)*F1d   !!!  Check
...
cc          WSQP = WSQ + XX(ISM) * PF * DW2DPF - es * dw2des
        do ism = 1,99
          ...
          IF(WSQP.GT. 1.159) THEN
            call CHRISTY507dr(Wsqp,Qsq,F1pp,Rc,sigtp,siglp)
            call resmodddr(wsqp,qsq,xvald0,F1dp)
            F1d = F1d + F1dp * Fyuse
            F1p = F1p + F1pp * Fyuse
...
cc        if(W .GT. 1.3) 
     ...
     >       W1=W1*(1.0+(P(20)*W+P(21)*W**2)/(1.0+P(22)*QSQ))**2
        ...
        CALL MEC2009dr( Z , A , qsq , wsq , F1M )
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      SUBROUTINE MEC2009dr(z,a,q2,w2,f1)
...
c       write(*,*) 'vahe1= ', A, W*W, Q2, f1corr
      ...
      SUBROUTINE F1F2QE09dr(Z, A, qsq, wsq, F1, F2)
c
...
c    nicely with Osipenko et al data from CLAS, up to 5 GeV**2
      ...
      REAL*8 P(0:23)
      COMMON/parcorrdr/P
      REAL*8 Z, A, avgN, F1, F2, wsq, qsq, wsql,nuloc,qvl,fac
...
cc      GMN = (1.+2.384*QSQ/(QSQ+11.92))*(1.-0.118*QSQ)*GMN       !!! Check
      GMN = (1.+0.3321*QSQ*QSQ/(QSQ*QSQ+0.9937))*(1.-0.1038*QSQ)*GMN   
c      GMN = 1.08*(1.-0.063*QSQ)*GMN 

c      GEN = 1.0*(1.-0.25*QSQ)**2*GEN

CCCC  Put in new FF parameterizations from JRA   - MEC  CCCCC

        GMP = RMUP/(1.-0.29940*Q+5.65622*QSQ-5.57350*Q3+      !  with Hall A data  !
     &         6.31574*Q4-1.77642*Q4*Q+0.30100*Q4*QSQ)
        GEP = 1./(1.-0.31453*Q+7.15389*QSQ-14.42166*Q3+
     &         23.33935*Q4-14.67906*Q4*Q+4.03066*Q4*QSQ)


c        GMN = RMUN /(1. + QSQ / 0.71)**2.2

c        if(QSQ.GT.1..AND.QSQ.LT.8.) then 
c          GMN = 1.065*(1.-0.065*sqrt(QSQ))*GMN
c        endif
...
c this version!
          w2p = (amd + nuloc - amp )**2 - 
     >      qvl**2 + 2. * qvl * pz - avp2(izp)

c          w2p = (w2p - amp*amp)*0.9 + amp*amp  !!! check
...
c        wsq = (wsq-amp*amp)*0.985+amp*amp
...
c      y = (wsq -amp**2) / qv
c      fy = fy*(1.5*abs(y)+1.5*y*y)     CC  Added by MEC 3/5/2011  CCC

      F2 = nuloc * FY * (nuL * GL + nuT * GT)
      F1 = amp * FY * GT / 2.
      ...
      if(F1.LT.0.0) F1 = 0.
      if(nuloc.gt.0. .and.f1.gt.0.) then
        R = (F2 / nuloc) / (F1 / amp) * (1. + nuloc**2 / qsq) - 1.0
      else
        r = 0.4/qsq
      endif
...
c    c       3.1914e-01,  -1.8657e-01,   3.2746e+01,   4.9801e-03/
     ...
     c       2.4902e-01,  -1.3728e-01,   2.9201e+01,   4.9280e-03/
      ...
      subroutine pinddr(W2,Q2,F1,R,sigt,sigl)
...
c         w2p = (w2p - am)*1.08 + am  !!! check


        if(w2p.gt.1.155) then
          call CHRISTY507dr(W2p,Q2,F1p,Rp,sigtp,siglp)
...
c         w2p = (w2p - am)*1.08 + am  !!! check
      ...
      subroutine resddr(q2,w2,xval,F1)
...
c         w2p = (w2p - am)*1.08 + am  !!! check
        ...
        if(w2p.gt.1.155) then
          call resmodddr(w2p,q2,xval,sigp)
...
! took out again, then put back in again
      call mecdr(1.D0,2.D0,q2,w2,f1m,f2m,xval)
      if(usemec) f1 = f1 + f1m
      return
      end

      subroutine resderdr(q2,w2,xval,F1er)
...
! add overall Hall C norm. err
      ...
      SUBROUTINE RESMODDdr(w2,q2,xval,sig) 
...
CCC  Put (F1p + F1n)/2 correction here - MEC, 3/19/2011   CCC

c      F1 = F1*(1.+0.000125*w2*w2*w2*sqrt(w2)) 

      F1 = F1*(1.+0.0009*w2*w2*sqrt(w2)) 

c      F1 = F1*(1.+0.0009*w2*sqrt(w2)) 
...
c Fit to proton F1, R, sigma_T, and Sigma_L from
      ...
      SUBROUTINE christy507dr(W2,Q2,F1,R,sigt,sigl)
c   M.E. Christy and P.E. Bosted, ``Empirical Fit to Precision 
c    Inclusive Electron-Proton Cross Sections in the Resonance Region'',
c    (arXiv:0712.3731). To be submitted to Phys. Rev. C.
     ...
     & 0.18951E+01,0.51376E+00,0.00000E+00,0.42802E+01,0.00000E+00 /
      ...
      call resmod507dr(1,w2,q2,xval1,sigT)
      call resmod507dr(2,w2,q2,xvalL,sigL)
      ...
      SUBROUTINE RESMOD507dr(sf,w2,q2,xval,sig) 
...
!---------------------------------------------------------------------
      ...
      REAL FUNCTION fitemcdr(X,A,GOODFIT)       
!---------------------------------------------------------------------  
...
!Term    Coeficient     Error                                          
     ...
     >  5.04268396D-03,    1.406D-03   /                                
      ...
      subroutine mecdr(z,a,q2,w2,f1,f2,xval)
! fit to low q2 dip region: purefly empirical
! assume contribution is pure transverse
      implicit none
